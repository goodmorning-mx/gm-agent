            ...
            for call in response.tool_calls:
                ...
                if result.requires_confirmation:
                    if self.confirmation is None or not await self.confirmation(tool_call.name, arguments):
                        ...
                        conversation.messages.append(Message("tool", json.dumps({"status": "confirmation_required", "tool": tool_call.name}), tool_call_id=call["id"], name=call["name"]))
                        continue
                    result = await self.registry.invoke(ToolCall(tool_call.name, arguments, confirmed=True), context)
                conversation.messages.append(Message("tool", json.dumps(result.content, default=str), tool_call_id=call["id"], name=call["name"]))
